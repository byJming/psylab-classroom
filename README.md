...
> **在线体验：https://byjming.github.io/psylab-classroom/**（推荐桌面 Chrome / Edge 最新两个大版本，配实体键盘）

PsyLab 是面向中国大陆高校心理学教学与公众科学体验的**开源、浏览器原生**心理实验平台：打开网页即可运行经典实验，完成练习与正式试次，查看本次任务的描述性指标，并带走可解释、可导入的本地结果包。**不需要账号、不依赖云端数据库、不使用第三方追踪。**

![PsyLab 首页：实验目录优先展示，八个开放层实验一目了然](docs/images/home.png)

> 当前版本为公开发布的 **beta** 版本：八个实验均可完整体验与课堂使用，但按路线图规定，完成两次真实课程场景试用反馈前，实验状态不标记为 `stable`。

---

## 这个平台能做什么

- **体验馆**：公众选择一个经典实验，几分钟完成，结果只描述本次任务表现，不是诊断或能力评估。
- **课堂模式**：教师生成本地会话配置分发给学生；学生在自己浏览器完成实验并下载结果包；教师在本地批量导入、校验与汇总，全程不经过任何服务器。
- **实验包仓库**：每个实验都公开定义（Definition）、参数 schema、程序化刺激、指标说明、许可清单和固定随机种子样例，可复现、可审阅。

PsyLab **不是**心理诊断工具、人格/智商测试站，也不是云端数据收集平台。

## 当前实验目录

八个 `open / beta` 官方实验，全部使用程序化原创刺激、中文说明、描述性结果：
...
更多候选范式与筛选标准见 [实验目录与筛选标准](docs/experiment-catalog.md)。
...
所有校验、去重与导出都在教师本机完成，结果不上传任何服务器。

## 数据与隐私

- 数据默认留在完成实验的浏览器（IndexedDB），导出为本地文件；平台不收集任何数据。
- 使用匿名参与者代码；请勿在代码中填写真实身份信息。
- 结果包不包含姓名、学号、IP、设备指纹或健康信息。
- 结果只描述本次任务表现，不输出诊断、人格类别、智力等级或医疗建议。
